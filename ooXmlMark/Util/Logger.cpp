...
Logger::Logger(std::string LogFilePath, unsigned int RotateSize)
{
   m_LogFilePathj = LogFilePath;
   m_RotateLength = RotateSize;

   std::ifstream ifs;
   ifs.open(m_LogFilePathj.c_str(), std::ifstream::binary);
   if (ifs.is_open()){
	   ifs.seekg(0, ifs.end);
	   m_CurrFileLength = (unsigned int)ifs.tellg();

	   if (m_CurrFileLength > m_RotateLength){
		   m_bTruncateFile = true;
		   m_CurrFileLength = 0;
	   }
   ...
   if (m_bTruncateFile){
	   m_File.open(m_LogFilePathj.c_str(), ios::out | ios::trunc);
   }
   else{
	   m_File.open(m_LogFilePathj.c_str(), ios::out | ios::app);
   }
...
Logger::Logger(std::string LogFilePath, LOG_TYPE LogType, LOG_LEVEL LogLevel, unsigned int RotateSize){
...
Logger* Logger::getInstance(std::string LogFilePath, unsigned int RotateSize) throw ()
{
   if (m_Instance == 0) 
   {
	   m_Instance = new Logger(LogFilePath, RotateSize);
   }
   return m_Instance;
}

Logger* Logger::getInstance(std::string LogFilePath, LOG_TYPE LogType, LOG_LEVEL LogLevel, unsigned int RotateSize) throw (){

	if (m_Instance == 0)
	{
		m_Instance = new Logger(LogFilePath, LogType, LogLevel, RotateSize);
	}
...
bool Logger::cmpInstanceFile(std::string LogFilePath){

	if (m_LogFilePathj.compare(LogFilePath)){
		return true;
	}
	else{
		return false;
...
void Logger::logIntoFile(std::string& data)
{
   ...
   //stringStream << "$##### " << m_bTruncateFile << " +++++++++++++ " << getCurrentTime() << " " << data << endl;
   //m_File << stringStream.str() << endl;
   //m_CurrFileLength += stringStream.str().length();

   m_File << data << endl;
   m_CurrFileLength += data.length();
...
string Logger::getCurrentTime()
{
   ...
   char szTime[64];
   ctime_s(szTime, 64, &now);
   // Convert current time to string
   currTime.assign(szTime);
