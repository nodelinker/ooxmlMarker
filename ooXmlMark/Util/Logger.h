...
namespace CPlusPlusLogging
{
   ...
   {
      public:
		  static Logger* getInstance(std::string LogFilePath = "MyLogFile.log", unsigned int RotateSize = 1024 * 1024 * 10) throw ();
		 static Logger* getInstance(std::string LogFilePath, LOG_TYPE LogType, LOG_LEVEL LogLevel, unsigned int RotateSize = 1024 * 1024 * 10) throw ();

		 bool cmpInstanceFile(std::string LogFilePath);
      ...
      protected:
         Logger(std::string LogFilePath, unsigned int RotateSize);
		 Logger(std::string LogFilePath, LOG_TYPE LogType, LOG_LEVEL LogLevel, unsigned int RotateSize);
...
#endif
		 unsigned int			 m_CurrFileLength;

		 bool				     m_bTruncateFile = false;
		 unsigned int			 m_RotateLength;
         LogLevel                m_LogLevel;
         LogType                 m_LogType;
		 std::string			 m_LogFilePathj;
   };
